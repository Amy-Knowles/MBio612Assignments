...
x <- 1:5
formatted_x <- sprintf("\033[1m%s\033[0m is emphasized.", x)
cat("Method 4: Using the sprintf function\n")
cat(formatted_x, "\n\n")
```
...
* Use the variable `sec_per_min` to compute a new variable called `sec_per_hour` representing the number of seconds per hour
  * `sec_per_hour` is simply the number of `sec_per_min` multiplied by 60.
...
# Create an expression with the calculated value
expr <- expression(paste("The result of ", sec_per_hour, " * ", 24, " is ", sec_per_day))

# Evaluate and print the expression
cat("Expression:", deparse(expr), "\n")
cat("Evaluated value:", eval(expr), "\n")
...
* What does the following create? ##a character vector
  * Specifically, what is the of the data it contains?

`c(1, 2, 3, "Hi")` 

* Would the following be valid? ##no
...
* Hint: We know that we can change the default behavior of a function by changing the default parameters.
  * Which default parameter (param) controls the `x-axis`, `y-axis`, and plot labels? ##xlab, ylab, and main
  * Which default parameter controls the size of the symbol (dot here)? ##size
...
qplot(x = x_axis, y = y_axis, geom = "point", main = "Scatter Plot of x_axis and y_axis")

qplot(x_axis, y_axis, geom = "point", 
      xlab = "My x_axis", ylab = "My y_axis",
      main = "My Amazing Plot of x_axis vs y_axis",
      size = 3) +
